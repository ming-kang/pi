...
> A personal `@astralyn/pi` distribution based on [earendil-works/pi](https://github.com/earendil-works/pi). It is not an upstream release.

This closed repository contains only the standalone coding-agent package maintained by its owner and invited Contributors. It does not accept external issues or pull requests. Product usage and public APIs are indexed in [`docs/index.md`](docs/index.md); distribution architecture, bundled behavior, maintenance, and publishing are under [`docs/distribution/`](docs/distribution/README.md).

## What this distribution changes

- Native tool calls use a consistent `●` call / `│` result presentation with bounded collapsed output.
- Long tool loops auto-compact after a complete tool batch and before the next provider request, continuing the same run when the rebuilt context is safe and failing closed otherwise.
- Personal workflow extensions are bundled: `deepwiki`, `question`, `todo`, `rewind`, `router`, and `statusline`.
- `ice-cream-dark` and `ice-cream-light` are bundled themes.

The AI, Agent core, and TUI runtimes are exact upstream npm dependencies rather than locally forked workspaces. See the [architecture](docs/distribution/architecture.md) and [maintenance guide](docs/distribution/maintenance.md) for that boundary.
...
This distribution checks the npm registry for the latest `@astralyn/pi` version. Disable the startup check with `PI_SKIP_VERSION_CHECK=1`.

The `enableInstallTelemetry` setting name is retained for configuration compatibility, but it now controls only optional provider attribution headers for supported OpenRouter, Cloudflare, and NVIDIA NIM requests. The Fork does not send install or update pings to `pi.dev`. Set it to `false` in `settings.json`, or set `PI_TELEMETRY=0`, to disable those headers.

Use `--offline` or `PI_OFFLINE=1` to disable startup network operations, including the npm version check, model refresh, and package update checks.
...
| `PI_OFFLINE` | Disable startup network operations, including npm version checks, package update checks, and model refresh |
| `PI_SKIP_VERSION_CHECK` | Skip the Pi version update check at startup. This prevents the npm registry latest-version request |
| `PI_TELEMETRY` | Override optional provider attribution headers. Use `1`/`true`/`yes` to enable or `0`/`false`/`no` to disable. This does not disable version checks |
...
This repository is developed privately by its owner and invited Contributors. See [docs/development.md](docs/development.md) for setup and debugging notes. Distribution-specific development rules are in [AGENTS.md](AGENTS.md).
