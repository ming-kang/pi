...
**Running** also lists live foreground executions so the currently running command or group can be watched or stopped. **Finished** holds background shell tasks and all Subagent groups, including groups that ran in the foreground. Foreground rows carry an `fg` tag in either section. Settled foreground shell commands deliver inline in the transcript, leave the panel, and are counted in the title as `N foreground shells hidden`. A selected shell row is the exception — it never disappears mid-watch when its execution settles, and leaves once the selection moves away.
