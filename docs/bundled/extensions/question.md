...
- `preview` shows focused single-select content beside the focused choice on wide terminals and directly beneath it on narrow terminals. Preview height is capped and reports hidden lines. Previews on multi-select questions are rejected with a `preview_multiselect` error.
...
- Short terminals use a height-bounded vertical viewport. Moving option focus keeps that option and its preview visible; long Review answers can be scrolled with the configured up/down bindings while Enter still submits and Esc returns to editing.
