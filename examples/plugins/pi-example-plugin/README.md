...
From this directory's parent of parents (the repository root):

```bash
PI_EXPERIMENTAL=1 pi experimental server \
  -e "$PWD/examples/plugins/pi-example-plugin"
```

Alternatively, a client can select the plugin for the Session it creates or resumes on one local server:

```bash
PI_EXPERIMENTAL=1 pi experimental client \
  -e "$PWD/examples/plugins/pi-example-plugin"
```
