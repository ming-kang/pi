...
- Auto-compaction now triggers at a percentage of the context window (`compaction.triggerPercent`, default 85) instead of a fixed 16384-token reserve, so large-window models compact at ~85% usage rather than ~98%. Upstream's `compaction.reserveTokens` setting no longer exists and is ignored when present; the summary length budget keeps the previous 16384-token value as an internal constant.
