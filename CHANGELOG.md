...
- Improved `/bg` with explicit configurable list/preview focus, independent pane scrolling and paging, retained per-row preview positions, visible ranges and shell following/browsing state, sticky metadata/diagnostics, and a width-safe rounded split-panel frame. Navigation and closing remain observation-only; stopping a worker still targets its whole group.
