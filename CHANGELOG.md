...
- Fetch Models copes with Anthropic-compatible gateways that serve their catalog OpenAI-style only: a `401`, `403`, or `404` on the Anthropic path triggers one retry with `Authorization: Bearer`, at the same URL or with a trailing `/anthropic` removed. Anthropic catalogs also follow their `has_more` marker through `after_id` (up to five pages) instead of reporting the default 20-entry page as partial.
...
- Providers now receive a normalized `TranscriptContext`: custom providers must read the prompt and tools with `getCurrentSystemPrompt()` and `getCurrentTools()` instead of `context.systemPrompt` / `context.tools`.
- `user_bash` now fails closed: a handler error or an invalid defined result aborts the command instead of falling back to local execution.
- Built-in `read`, `bash`, `powershell`, `edit`, and `write` tools use strict-prefer JSON-schema sampling by default, without `PI_EXPERIMENTAL`.
- `--resume` results now appear progressively and cancel outstanding transcript reads after selection; `--continue` stops at the newest matching session header.
- Replaced the optional native clipboard dependency with bundled command helpers and the TUI's native clipboard; `@mariozechner/clipboard` is no longer installed.
- Deferred the extension compiler and bundled virtual modules until a filesystem extension loads, and upgraded the seven `@earendil-works/*` dependencies plus chalk, grok-mermaid, ignore, minimatch, semver, typebox, and undici.
...
- Signal-terminated shell commands are now reported as failures instead of successes with partial output, in foreground and background execution alike.
- Fixed cancellation races that could start automatic compaction, leave stale retry state, or miss cancellation while waiting for summarization authentication; tree navigation during compaction now reports an error instead of replacing its progress UI.
- Fixed repeated Anthropic thinking-drop notices for the same blocks, extension tools registered without a parameter schema, direct RPC `steer`/`follow_up` bypassing extension `input` handlers, and fullscreen reserving a blank row for zero-row custom footers.
- Retired this distribution's trailing-tool-result compaction cut-point fix in favor of upstream's equivalent.

### Removed

- Removed the `kimi-deferred-tools.ts` example along with upstream's Kimi deferred-tool compat field.
...
- Auto-compaction now triggers at a percentage of the context window (`compaction.triggerPercent`, default 85, clamped to 20–95) instead of a fixed 16384-token reserve, so large-window models compact at ~85% usage rather than ~98%. Upstream's `compaction.reserveTokens` setting no longer exists and is ignored when present; the previous 16384-token internal summary reserve remains an upper limit.
- Status line context usage now turns warning above 40% and error above 80% of the window (previously 70%/90%), ahead of the 85% default auto-compaction trigger.
...
- Fixed low compaction thresholds keeping the entire conversation because the default recent-message budget exceeded the trigger. Both automatic and manual compaction now adapt retention and summary output limits to the model's window and effective trigger percentage. Settings readers report the same clamped percentage used during execution.
...
- Changed interactive Background completion cards to read self-contained structured snapshots with explicit shell identity, commands, independent diagnostics/output, worker reports and source truncation flags. The same snapshot produces bounded model prose, with a report allowance for every worker. Completion details use version 1; retained task results use version 2. Earlier result records are not migrated/restored, and text-only completion cards use bounded plain details.
...
- Rewrote the model-facing bash/powershell, Subagent and bg guidance around the foreground/background choice: foreground blocks until results, background returns a task reference immediately and completion is delivered automatically as a notification, with bg read/wait/kill to inspect, await or stop. Handoff results state the automatic delivery, and the shell description no longer restates the subagent background restriction already enforced by the execution-role rejection.
- Scoped `/bg` and `bg list` to backgrounded work: settled foreground executions leave the panel's Finished section and model-facing listings (their results already live inline in the transcript) with a counted omission note, running foreground executions stay listed with an `fg` tag, and the status bar counts background tasks only. A selected panel row never disappears when its foreground execution settles; it leaves once the selection moves away.
- Reworked `/bg` Subagent worker presentation: groups list one compact line per worker (marker, label, status and task description) instead of a wrapping everything-line, and worker detail shows model and usage on separate rows that wrap rather than truncate.
...
- Fixed command `Output:` lines, report headings/fences, error-wrapper examples and literal truncation notices being mistaken for completion metadata. Cards now consume explicit fields and remain replayable after task eviction or log expiry.
...
- Redesigned the bundled Subagent collapsed view around a compact flow grid: each task renders as one status-marked cell — a breathing dot-to-star bloom spinner while running, a static muted `○` while queued, and `✓`/`×`/`■` when settled — showing only its ordinal and profile, cells wrap at their boundaries with uniform widths so continuation rows stay column-aligned, and the call header settles on a quiet `● Subagent` title. Batch elapsed time and aggregate cost moved into a `── Batch` summary line at the top of the expanded view; activity summaries, per-run durations, failure details, and retry countdowns are no longer echoed in collapsed rows. Live refresh runs at the spinner cadence while a task is running and the card is collapsed, falling back to a one-second cadence when expanded or settled, and switching between the two takes effect immediately when the card is expanded or collapsed.
## [0.84.10] - 2026-08-24
