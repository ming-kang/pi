...
describe("tool cwd resolution", () => {
	...
	it("bash uses ctx.cwd when provided", async () => {
		...
		const result = await tool.execute(
			"test-bash-ctx-cwd",
			// Git Bash's pwd prints a POSIX path (/tmp/...); -W prints the Windows path.
		...
		const output = getTextOutput(result);
		// Git Bash prints MSYS-style paths (C:/...) even with pwd -W.
		const expected = process.platform === "win32" ? testDir.replaceAll("\\", "/") : testDir;
