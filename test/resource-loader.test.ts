  ...
  pi.registerCommand("deploy", {
	...
	describe("loadProjectContextFiles - nested worktree dedup", () => {
		...
		it.skipIf(process.platform !== "win32")(
			"should compare nested worktree paths case-insensitively on Windows",
			() => {
				const { main, worktree, worktreeSrc } = setupNestedWorktree();
				writeFileSync(join(main, "AGENTS.md"), "main repo instructions");
				writeFileSync(join(worktree, "AGENTS.md"), "worktree instructions");
				const alternateDriveCase = worktreeSrc.replace(/^([A-Za-z]):/, (drive) =>
					drive === drive.toLowerCase() ? drive.toUpperCase() : drive.toLowerCase(),
				);
				expect(alternateDriveCase).not.toBe(worktreeSrc);

				const files = loadProjectContextFiles({ cwd: alternateDriveCase, agentDir });
