...
function createNpmPrefixInstall(
	template = "pi-prefix-",
	layout: "inferred" | "windows-global" = "inferred",
): { prefix: string; packageDir: string } {
	const prefix = mkdtempSync(join(tmpdir(), template));
	const root = layout === "windows-global" ? join(prefix, "node_modules") : join(prefix, "lib", "node_modules");
	const scopeDir = join(root, "@astralyn");
	const packageDir = join(scopeDir, "pi");
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@astralyn", "pi");
	mkdirSync(packageDir, { recursive: true });
	...
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(root, ".pnpm", "@astralyn+pi@0.81.1-1", "node_modules", "@astralyn", "pi", "dist", "cli.js"));
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@astralyn", "pi");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@astralyn", "pi", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@astralyn/pi")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @astralyn/pi",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getUpdateInstruction("@astralyn/pi")).toBe(
			"Update @astralyn/pi using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@astralyn/pi");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@astralyn/pi"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @astralyn/pi`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@astralyn/pi", undefined, {
			packageName: "@astralyn/pi",
			installSpec: "@astralyn/pi@1.2.3",
		});

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@astralyn/pi@1.2.3"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @astralyn/pi@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		const { prefix } = createNpmPrefixInstall(
			"pi-prefix-",
			process.platform === "win32" ? "windows-global" : "inferred",
		);

		const command = getSelfUpdateCommand("@astralyn/pi", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @astralyn/pi`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@astralyn\\pi";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@astralyn/pi")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @astralyn/pi",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@astralyn/pi"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @astralyn/pi",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@astralyn/pi";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@astralyn", "pi");
