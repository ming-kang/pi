...
// This distribution publishes the package as @astralyn/pi; plugins written
// against it use that name for the same experimental plugin API.
const DISTRIBUTION_PLUGIN_API = "@astralyn/pi/experimental/plugin";
...
function resolvePluginExternal(specifier: string): string | undefined {
	if (specifier !== PI_PLUGIN_API && specifier !== DISTRIBUTION_PLUGIN_API) return undefined;
