...
export class AgentSessionRuntime {
	...
	async switchSession(
		...
		try {
			...
			// Resuming the current file must include usage persisted during teardown.
	...
	async fork(
		...
		try {
			...
			const forkingCurrentLeaf = targetLeafId !== null && targetLeafId === sessionManager.getLeafId();
			await this.teardownCurrent("fork", sessionManager.getSessionFile());
			if (forkingCurrentLeaf) targetLeafId = sessionManager.getLeafId();
			if (!targetLeafId) {
