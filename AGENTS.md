...
- Consume AI, Agent core, Client, Protocol, and TUI through the exact `@earendil-works/pi-ai`, `@earendil-works/pi-agent-core`, `@earendil-works/pi-client`, `@earendil-works/pi-protocol`, and `@earendil-works/pi-tui` npm dependencies; experimental source also consumes exact `@earendil-works/chord` and `@earendil-works/pi-server` dependencies. Classify production and development dependencies by their consumers as described in [Architecture](maintainers/architecture.md#dependency-boundary). Never vendor, patch, monkey-patch, or recreate them.
...
- `npm run build` emits the full `tsc` output, then builds Node entrypoints and shared chunks under `dist/bundle/` via `scripts/build-bundle.mjs`. Keep `PI_BUNDLED_NODE` defined so user-extension loading uses embedded virtual modules, and emit OAuth, Bedrock, and image-worker modules beside their bundled loaders. Follow the released upstream bundling strategy using installed npm artifacts, without modifying upstream library implementations.
...
- [Architecture](maintainers/architecture.md) — dependency scope and subsystem ownership.
- [Dependency maintenance](maintainers/dependencies.md) — installation checks, age exceptions, and lockfile review.
