...
const distCliPath = join(packageDir, "dist", "bundle", "cli.js");
...
function printHelp() {
	console.log(`Usage:
  node scripts/profile-node.mjs [options]

Profiles @astralyn/pi startup with the runtime selected below:
- npm run profile:tui     -> builds the package and profiles TUI startup with Node
- npm run profile:rpc     -> builds the package and profiles RPC startup with Node
...
Options:
  ...
  --skip-build           Reuse the current dist/bundle/cli.js without rebuilding first (Node only)
...
async function runBuild() {
	process.stdout.write("Building @astralyn/pi...\n");
...
async function main() {
	...
	if (runtime === "bun") {
		process.stdout.write(
			`Using Bun runtime with ${options.mode === "rpc" ? "src/cli.ts --mode rpc" : "src/cli.ts"}\n`,
		);
	}
