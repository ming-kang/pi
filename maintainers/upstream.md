...
- **Terminal output and private field (high risk):** Coalesced writes support IME stability and guarded scrollback preservation. Only rewrite recognized full-render frames; unknown shapes, size changes, or invalid state must use the safe fallback. Viewport preservation relies on the private `TUI.previousViewportTop` field, which must be validated and never treated as a supported pi-tui API. Test final-screen equivalence and scrollback, plus Windows Terminal IME, scrolling, and tall expanded results.
