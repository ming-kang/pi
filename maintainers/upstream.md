...
- **Experimental Chord runtime (v0.85.0):** The `src/experimental/` tree pulls in exact `@earendil-works/chord` and `@earendil-works/pi-server` dependencies (upstream references pi-server only as a workspace sibling, so the distribution must declare it). The experimental durable server is POSIX-only by upstream design (`process.getuid` guard in `src/experimental/server.ts`); its tests skip on native Windows and are covered by Ubuntu CI. `src/experimental/plugins/bundled.ts` additionally maps this distribution's `@astralyn/pi/experimental/plugin` specifier for plugin external resolution.
...
- **Session-owned background execution:** The core Background service (`src/core/background/`) owns admission, settlement persistence, and exactly-once completion delivery for `background: true` shell commands (bash and PowerShell share it) and whole subagent groups; the bundled background extension is an observer/controller only. Completion delivery waits for a safe idle boundary, warns on failure, and retries on the next user prompt; tree navigation cancels off-branch executions and revives their undelivered completions on return. Upstream has no equivalent; merge upstream changes to `agent-session*.ts`, `tools/bash.ts`, `tools/output-accumulator.ts`, and `usage-totals.ts` by hand around this wiring.
