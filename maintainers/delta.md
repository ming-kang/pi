...
**Scrollback preservation.** pi-tui's `fullRender(clear)` opens with `2J H 3J`, wiping terminal scrollback, then replays the entire transcript. Content-driven triggers (a line changing above the viewport during streaming markdown reflow) fire this routinely, erasing pre-session shell history and — in terminals that clamp the scroll offset when scrollback vanishes, notably Windows Terminal — yanking a scrolled-up viewport to the top of the buffer (upstream [#6502](https://github.com/earendil-works/pi/issues/6502) open, [#5576](https://github.com/earendil-works/pi/issues/5576), [#6050](https://github.com/earendil-works/pi/issues/6050); upstream declined the equivalent fix [#4204](https://github.com/earendil-works/pi/pull/4204)). Frames matching `?2026h 2J H 3J <lines> ?2026l` are rewritten to overwrite the visible screen in place — home, erase-and-rewrite each row (`2K`), erase below (`J`) — with only the bottom viewport-height rows, producing a byte-identical final screen while leaving scrollback and the reader's scroll position untouched. ED 2 is avoided entirely in the rewritten frame: conhost/Windows Terminal implement it by scrolling the screen contents into scrollback (cls compatibility), so a `2J`-based repaint without the `3J` wipe would stack a duplicate screenful into history on every content-driven redraw (runaway buffer growth under Ctrl+O-expanded tool output during streaming). Width-change redraws keep the upstream wipe (re-wrapped content invalidates old scrollback; detected by comparing `columns` across writes), kitty-graphics frames pass through untouched, and any frame that does not match the expected shape passes through unchanged, so an upstream reshape degrades back to stock behavior. Accepted trade-offs: scrollback keeps the historical rendering of lines that later changed above the viewport, and a redraw that coincides with a line-count change can leave a few-line seam at the scrollback boundary.
...
- Upstream assumptions: `TUI` accepts any `Terminal` implementation; `ProcessTerminal` keeps `write`, `moveBy`, cursor visibility, and clear helpers overridable; `ProcessTerminal.stop()` writes its teardown sequences directly to stdout (the override flushes pending output first); `fullRender(clear)` frames start with `?2026h` + optional kitty deletes + `2J H 3J` and end with `?2026l`, with rows joined by CRLF. If upstream folds the cursor reposition into the frame or stops wiping scrollback, the corresponding rewrite becomes a no-op.
- Verify: `test/coalescing-terminal.test.ts`, `test/coalescing-terminal-scrollback.test.ts` (xterm-emulated: no `3J` emitted, shell history survives a content-driven full redraw, final screen matches the upstream-intended state); on Windows Terminal with a CJK IME, type into the editor and confirm the candidate window stays at the caret, and scroll up during a streaming response to confirm the viewport no longer jumps to the top.
